    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '0 16px' : '0 24px' }}>
      ...
      {wPreconKpis && (() => {
        ...
        const cards: Array<{ label: string; value: number | string; color: string; Icon: any; sub: string }> = [
          ...
          { label: 'READY TO START', value: ready, color: '#c88c00', Icon: CheckCircle2, sub: 'design complete' },
          // Task-based counts
      ...
      {wAllTasks && tasks.length > 0 && (() => {
        ...
        const filteredJobs = taskSearch.trim()
          ...
          : sortedJobs;
        return (
          <div style={{ background: '#f8f6f3', border: '1px solid rgba(200,140,0,0.15)', borderRadius: 8, padding: '10px 12px', marginBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <ClipboardList size={16} style={{ color: '#c88c00' }} />
                <span style={{ fontSize: 14, fontWeight: 700, color: '#1a1a1a', letterSpacing: '0.02em' }}>My Tasks by Job ({filteredTasks.length})</span>
                <span style={{ fontSize: 11, color: '#5a5550', marginLeft: 4 }}>Overdue thru {rangeEndStr}</span>
              </div>
              <button
                onClick={() => {
