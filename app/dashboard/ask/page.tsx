  ...
  return (
    <>
      {elements.map((el: any) => {
        if (el.type === 'code') {
          return (
            <div key={el.key} className="relative mt-3 mb-3 rounded-lg" style={{ background: '#ffffff', border: '1px solid rgba(200,140,0,0.15)' }}>
              <div className="flex items-center justify-between px-3 py-1.5" style={{ borderBottom: '1px solid rgba(200,140,0,0.1)', background: 'rgba(200,140,0,0.04)' }}>
                ...
                <button
                  ...
                  style={{ color: '#c88c00', background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.2)' }}
                >Copy</button>
              </div>
              <pre className="px-3 py-2 text-xs overflow-x-auto whitespace-pre-wrap" style={{ color: '#3a3530', fontFamily: 'ui-monospace, SFMono-Regular, "SF Mono", Menlo, monospace', lineHeight: '1.5' }}>{el.content}</pre>
            </div>
          );
        }
        if (el.type === 'h2') return <div key={el.key} className="font-bold mt-3 mb-1" style={{ color: '#c88c00', fontSize: '0.9rem' }} dangerouslySetInnerHTML={{ __html: el.html }} />;
        if (el.type === 'h3') return <div key={el.key} className="font-semibold mt-2 mb-0.5" style={{ color: '#1a1a1a', fontSize: '0.85rem' }} dangerouslySetInnerHTML={{ __html: el.html }} />;
...
function TaskConfirmCard({ data, onPhaseChange }: { data: TaskConfirmData; onPhaseChange?: (phase: string) => void }) {
  ...
  return (
    <div className="mt-3 rounded-lg" style={{ background: '#ffffff', border: '1px solid rgba(201,168,76,0.25)' }}>
      <div className="px-3 py-2 flex items-center gap-2 rounded-t-lg" style={{ background: 'rgba(201,168,76,0.08)', borderBottom: '1px solid rgba(201,168,76,0.15)' }}>
        <span className="text-xs font-semibold" style={{ color: '#c88c00' }}>Task Preview</span>
      ...
      <div className="px-3 py-2.5 space-y-1.5">
        {data.name && (
          <div className="flex items-start gap-2">
            ...
            <span className="text-sm font-medium" style={{ color: '#1a1a1a' }}>{data.name}</span>
          </div>
        )}
        <div className="flex items-start gap-2 relative">
          <span className="text-[10px] font-medium w-16 flex-shrink-0 pt-0.5" style={{ color: '#8a8078' }}>Phase</span>
          <button onClick={() => setEditingPhase(!editingPhase)} className="flex items-center gap-1 text-xs px-2 py-0.5 rounded transition-colors hover:brightness-125" style={{ color: phaseChanged ? '#22c55e' : '#c88c00', background: phaseChanged ? 'rgba(34,197,94,0.1)' : 'rgba(201,168,76,0.1)', border: '1px solid ' + (phaseChanged ? 'rgba(34,197,94,0.2)' : 'rgba(201,168,76,0.2)'), cursor: 'pointer' }}>
            {displayPhase} <ChevronDown size={10} className={`transition-transform ${editingPhase ? 'rotate-180' : ''}`} />
          </button>
          {editingPhase && (
            <div className="absolute left-16 top-7 z-50 rounded-lg shadow-lg" style={{ background: '#f8f6f3', border: '1px solid rgba(200,140,0,0.2)', minWidth: '200px', maxHeight: '280px', overflowY: 'auto' }}>
              {BKB_PHASES.map(phase => (
                <button key={phase} onClick={() => handlePhaseSelect(phase)} className="w-full text-left px-3 py-1.5 text-xs transition-colors hover:bg-black/5" style={{ color: phase === displayPhase ? '#c88c00' : '#1a1a1a', background: phase === displayPhase ? 'rgba(201,168,76,0.08)' : 'transparent' }}>
                  {phase}
                </button>
              ))}
            </div>
        ...
        {data.assignee && (
          <div className="flex items-start gap-2">
            ...
            <span className="text-xs" style={{ color: '#1a1a1a' }}>{data.assignee}</span>
          </div>
        )}
        ...
        {data.description && (
          <div className="flex items-start gap-2">
            ...
            <span className="text-xs leading-relaxed" style={{ color: '#3a3530' }}>{data.description}</span>
          </div>
        )}
      </div>
    </div>
            ...
            {convsLoading ? (
              ...
              conversations.map((conv) => (
                <button
                  ...
                  className="group w-full text-left px-2.5 py-2 rounded-lg mb-0.5 transition-colors hover:bg-black/5 flex flex-col relative"
                ...
                >
                  <span className="text-xs font-medium truncate w-full pr-6" style={{ color: conversationId === conv.id ? '#c88c00' : '#1a1a1a' }}>
                    {conv.title}
                  </span>
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <div className="mb-4 flex items-start justify-between">
          <div className="flex items-center gap-2">
            <button onClick={() => setShowSidebar(!showSidebar)} className="p-1.5 rounded-lg transition-colors hover:bg-black/5" style={{ color: '#8a8078' }} title={showSidebar ? 'Hide sidebar' : 'Show sidebar'}>
              {showSidebar ? <PanelLeftClose size={18} /> : <PanelLeft size={18} />}
            </button>
            <div>
              <h1 className="text-2xl font-bold" style={{ fontFamily: 'Georgia, serif', color: '#c88c00' }}>Ask Agent</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={forceSync} disabled={syncing} title="Sync GHL & JobTread data" className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs transition-all hover:brightness-110 disabled:opacity-40" style={{ background: '#f8f6f3', color: syncing ? '#c88c00' : '#8a8078', border: '1px solid rgba(200,140,0,0.12)' }}>
          ...
          <div className="relative" ref={dropdownRef}>
            <button type="button" onClick={() => setShowJobDropdown(!showJobDropdown)} className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs transition-colors" style={{ background: selectedJob ? 'rgba(201,168,76,0.12)' : '#f8f6f3', color: selectedJob ? '#c88c00' : '#8a8078', border: selectedJob ? '1px solid rgba(201,168,76,0.3)' : '1px solid rgba(200,140,0,0.12)', minWidth: '180px' }}>
            ...
            {showJobDropdown && (
              <div className="absolute right-0 top-full mt-1 rounded-lg shadow-lg overflow-hidden z-50" style={{ background: '#ffffff', border: '1px solid rgba(200,140,0,0.15)', maxHeight: '340px', width: '320px' }}>
                <div className="px-2 py-2" style={{ borderBottom: '1px solid rgba(200,140,0,0.08)' }}>
                  <input ref={jobSearchRef} type="text" placeholder="Type to search projects..." value={jobSearch} onChange={(e) => setJobSearch(e.target.value)} autoFocus className="w-full px-2 py-1.5 rounded text-xs outline-none" style={{ background: '#f8f6f3', color: '#1a1a1a', border: '1px solid rgba(200,140,0,0.15)' }} />
                </div>
                <button onClick={() => { setSelectedJob(null); setShowJobDropdown(false); setJobSearch(''); }} className="w-full text-left px-3 py-2 text-xs transition-colors hover:bg-[#f8f6f3]" style={{ color: '#8a8078', borderBottom: '1px solid rgba(200,140,0,0.08)' }}>
                ...
                <div className="overflow-y-auto" style={{ maxHeight: '240px' }}>
                  {jobsLoading ? (
                    ...
                    return filtered.map(job => (
                      <button key={job.id} onClick={() => { setSelectedJob(job); setShowJobDropdown(false); setJobSearch(''); }} className="w-full text-left px-3 py-2 text-xs transition-colors hover:bg-[#f8f6f3] flex flex-col" style={{ color: selectedJob?.id === job.id ? '#c88c00' : '#1a1a1a', background: selectedJob?.id === job.id ? 'rgba(201,168,76,0.06)' : 'transparent' }}>
        ...
        <div className="flex gap-2 mb-4">
          <button onClick={() => switchAgent('know-it-all')} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all" style={agentMode === 'know-it-all' ? { background: 'rgba(201,168,76,0.15)', color: '#c88c00', border: '1px solid rgba(201,168,76,0.4)', boxShadow: '0 0 12px rgba(201,168,76,0.1)' } : { background: '#f8f6f3', color: '#8a8078', border: '1px solid rgba(200,140,0,0.12)' }}>
            <Brain size={16} /> Know-it-All
          </button>
          <button onClick={() => switchAgent('project-details')} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all" style={agentMode === 'project-details' ? { background: 'rgba(201,168,76,0.15)', color: '#c88c00', border: '1px solid rgba(201,168,76,0.4)', boxShadow: '0 0 12px rgba(201,168,76,0.1)' } : { background: '#f8f6f3', color: '#8a8078', border: '1px solid rgba(200,140,0,0.12)' }}>
        ...
        <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-1">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 rounded-lg" style={{ background: '#f8f6f3', border: '1px solid rgba(200,140,0,0.08)' }}>
              <div className="w-16 h-16 rounded-full flex items-center justify-center mb-4" style={{ background: 'rgba(201,168,76,0.1)' }}>
                {agentMode === 'know-it-all' ? <Brain size={28} style={{ color: '#c88c00' }} /> : <FileSearch size={28} style={{ color: '#c88c00' }} />}
              </div>
              <h2 className="text-lg font-bold mb-2" style={{ color: '#c88c00' }}>{agentMode === 'know-it-all' ? 'BKB Project Assistant' : 'Project Specifications'}</h2>
              <p className="text-sm max-w-md text-center mb-6" style={{ color: '#8a8078' }}>
                ...
                {selectedJob && <span style={{ color: '#c88c00' }}>{' '}Currently focused on <strong>#{selectedJob.number} {selectedJob.name}</strong>.</span>}
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-lg w-full">
                {suggestions.map(s => (
                  <button key={s} onClick={() => setQuery(s)} className="text-left px-3 py-2 rounded-lg text-xs transition-colors hover:border-opacity-30" style={{ background: '#ffffff', color: '#8a8078', border: '1px solid rgba(200,140,0,0.08)' }}>
                    {s}
                  </button>
                ))}
              </div>
          ...
          {loading && (
            <div className="flex justify-start gap-2">
              ...
              <div className="px-4 py-3 rounded-lg flex items-center gap-2" style={{ background: '#f8f6f3', border: '1px solid rgba(200,140,0,0.08)' }}>
        ...
        <form onSubmit={handleSubmit} className="relative">
          {selectedJob && (
            <div className="absolute -top-6 left-0 text-[10px] px-2 py-0.5 rounded-t" style={{ color: '#c88c00', background: 'rgba(201,168,76,0.08)' }}>
              Focused: #{selectedJob.number} {selectedJob.name}
            </div>
          )}

          {uploadedFiles.length > 0 && (
            <div className="flex flex-wrap gap-1.5 px-3 py-2 rounded-t-lg" style={{ background: '#f8f6f3', borderTop: '1px solid rgba(200,140,0,0.12)', borderLeft: '1px solid rgba(200,140,0,0.12)', borderRight: '1px solid rgba(200,140,0,0.12)' }}>
              {uploadedFiles.map((file, idx) => (
                <div key={`${file.name}-${idx}`} className="flex items-center gap-1.5 px-2 py-1 rounded text-[11px]" style={{ background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.15)', color: '#c88c00' }}>
          ...
          <div className="relative">
            <textarea
              ...
              style={{ background: '#f8f6f3', color: '#1a1a1a', border: selectedJob ? '1px solid rgba(201,168,76,0.25)' : '1px solid rgba(200,140,0,0.12)', borderRadius: uploadedFiles.length > 0 ? '0 0 0.5rem 0.5rem' : '0.5rem', minHeight: '44px', maxHeight: '160px', overflowY: 'auto' }}
              disabled={loading}
            />
            <button type="button" onClick={() => fileInputRef.current?.click()} className="absolute left-2 top-1/2 -translate-y-1/2 p-1.5 rounded transition-colors hover:bg-black/5" style={{ color: uploadedFiles.length > 0 ? '#c88c00' : '#8a8078' }} title="Attach PDF, TXT, or MD file">
              <Paperclip size={16} />
            </button>
            <button type="submit" disabled={!canSend} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-lg transition-colors" style={{ color: canSend ? '#c88c00' : '#8a8078', background: canSend ? 'rgba(201,168,76,0.1)' : 'transparent' }}>
              <Send size={18} />
            </button>
          </div>
        </form>
