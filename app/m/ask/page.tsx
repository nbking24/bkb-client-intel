  ...
  return (
    <>
      {elements.map((el: any) => {
        if (el.type === 'code') {
          return (
            <div key={el.key} className="relative mt-2 mb-2 rounded-lg overflow-hidden" style={{ background: '#ffffff', border: '1px solid rgba(200,140,0,0.15)' }}>
              <div className="flex items-center justify-between px-3 py-1.5" style={{ borderBottom: '1px solid rgba(200,140,0,0.1)', background: 'rgba(200,140,0,0.04)' }}>
                <span className="text-[10px] font-medium" style={{ color: '#8a8078' }}>Markdown</span>
                <button onClick={() => navigator.clipboard.writeText(el.content)} className="text-[10px] px-2 py-0.5 rounded" style={{ color: '#c88c00', background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.2)' }}>Copy</button>
              </div>
              <pre className="px-3 py-2 text-xs overflow-x-auto whitespace-pre-wrap" style={{ color: '#3a3530', fontFamily: 'ui-monospace, SFMono-Regular, monospace', lineHeight: '1.5' }}>{el.content}</pre>
            </div>
          );
        }
        if (el.type === 'h2') return <div key={el.key} className="font-bold mt-3 mb-1" style={{ color: '#c88c00', fontSize: '0.85rem' }} dangerouslySetInnerHTML={{ __html: el.html }} />;
        if (el.type === 'h3') return <div key={el.key} className="font-semibold mt-2 mb-0.5" style={{ color: '#1a1a1a', fontSize: '0.8rem' }} dangerouslySetInnerHTML={{ __html: el.html }} />;
        ...
        if (el.type === 'hr') return <hr key={el.key} className="my-2" style={{ borderColor: 'rgba(200,140,0,0.15)' }} />;
...
function TaskConfirmCard({ data, onPhaseChange }: { data: TaskConfirmData; onPhaseChange?: (phase: string) => void }) {
  ...
  return (
    <div className="mt-2 rounded-xl" style={{ background: '#ffffff', border: '1px solid rgba(201,168,76,0.25)' }}>
      <div className="px-3 py-1.5 flex items-center gap-2 rounded-t-xl" style={{ background: 'rgba(201,168,76,0.08)', borderBottom: '1px solid rgba(201,168,76,0.15)' }}>
        <span className="text-[11px] font-semibold" style={{ color: '#c88c00' }}>Task Preview</span>
      ...
      <div className="px-3 py-2 space-y-1.5">
        {data.name && (
          <div><span className="text-[10px] font-medium" style={{ color: '#8a8078' }}>Name: </span><span className="text-[13px] font-medium" style={{ color: '#1a1a1a' }}>{data.name}</span></div>
        )}
        <div className="relative">
          <span className="text-[10px] font-medium" style={{ color: '#8a8078' }}>Phase: </span>
          <button onClick={() => setEditingPhase(!editingPhase)} className="inline-flex items-center gap-1 text-xs px-1.5 py-0.5 rounded transition-colors active:brightness-125" style={{ color: phaseChanged ? '#22c55e' : '#c88c00', background: phaseChanged ? 'rgba(34,197,94,0.1)' : 'rgba(201,168,76,0.1)', border: '1px solid ' + (phaseChanged ? 'rgba(34,197,94,0.2)' : 'rgba(201,168,76,0.2)'), cursor: 'pointer' }}>
            {displayPhase} <ChevronDown size={10} className={`transition-transform ${editingPhase ? 'rotate-180' : ''}`} />
          </button>
          {editingPhase && (
            <div className="absolute left-0 top-full mt-1 z-50 rounded-xl shadow-lg" style={{ background: '#f8f6f3', border: '1px solid rgba(200,140,0,0.2)', minWidth: '200px', maxHeight: '280px', overflowY: 'auto' }}>
              {BKB_PHASES.map(phase => (
                <button key={phase} onClick={() => handlePhaseSelect(phase)} className="w-full text-left px-3 py-2 text-xs transition-colors active:bg-white/5" style={{ color: phase === displayPhase ? '#c88c00' : '#1a1a1a', background: phase === displayPhase ? 'rgba(201,168,76,0.08)' : 'transparent' }}>
                  {phase}
                </button>
              ))}
            </div>
        ...
        {data.assignee && (
          <div><span className="text-[10px] font-medium" style={{ color: '#8a8078' }}>Assignee: </span><span className="text-xs" style={{ color: '#1a1a1a' }}>{data.assignee}</span></div>
        )}
        {(data.startDate || data.endDate) && (
          <div><span className="text-[10px] font-medium" style={{ color: '#8a8078' }}>Due: </span><span className="text-xs" style={{ color: '#1a1a1a' }}>{data.endDate || data.startDate}</span></div>
        )}
        {data.description && (
          <div className="pt-0.5"><span className="text-[10px] font-medium" style={{ color: '#8a8078' }}>Details: </span><span className="text-xs leading-relaxed" style={{ color: '#3a3530' }}>{data.description}</span></div>
        )}
      </div>
    </div>
  );
...
export default function MobileAskAgentPage() {
  ...
  return (
    <div className="flex flex-col h-[100dvh] relative" style={{ background: '#111', color: '#1a1a1a' }}>

      {/* ── Top bar ── */}
      <div className="flex items-center justify-between px-3 py-2 flex-shrink-0" style={{ background: '#ffffff', borderBottom: '1px solid rgba(200,140,0,0.1)' }}>
        <div className="flex items-center gap-2 min-w-0">
          <button onClick={() => setShowDrawer(true)} className="p-2 -ml-1 rounded-lg active:bg-white/5" style={{ color: '#8a8078' }}>
            <Menu size={20} />
          </button>
          <h1 className="text-base font-bold truncate" style={{ fontFamily: 'Georgia, serif', color: '#c88c00' }}>Ask Agent</h1>
        </div>
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          <button onClick={forceSync} disabled={syncing} className="p-2 rounded-lg active:bg-white/5" style={{ color: syncing ? '#c88c00' : '#8a8078' }}>
            <RefreshCw size={16} className={syncing ? 'animate-spin' : ''} />
          </button>
          <button onClick={handleNewChat} className="p-2 rounded-lg active:bg-white/5" style={{ color: '#c88c00' }}>
            <Plus size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 px-3 py-2 flex-shrink-0 overflow-x-auto" style={{ borderBottom: '1px solid rgba(200,140,0,0.06)' }}>
        <button onClick={() => switchAgent('know-it-all')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0"
          style={agentMode === 'know-it-all' ? { background: 'rgba(201,168,76,0.15)', color: '#c88c00', border: '1px solid rgba(201,168,76,0.4)' } : { background: '#f8f6f3', color: '#8a8078', border: '1px solid rgba(200,140,0,0.12)' }}>
        ...
        <button onClick={() => switchAgent('project-details')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap flex-shrink-0"
          style={agentMode === 'project-details' ? { background: 'rgba(201,168,76,0.15)', color: '#c88c00', border: '1px solid rgba(201,168,76,0.4)' } : { background: '#f8f6f3', color: '#8a8078', border: '1px solid rgba(200,140,0,0.12)' }}>
          <FileSearch size={13} /> Specs
        </button>
        <div className="h-4 w-px flex-shrink-0" style={{ background: 'rgba(200,140,0,0.12)' }} />
        <button onClick={() => setShowJobPicker(true)} className="flex items-center gap-1 px-3 py-1.5 rounded-full text-xs whitespace-nowrap flex-shrink-0"
          style={{ background: selectedJob ? 'rgba(201,168,76,0.1)' : '#f8f6f3', color: selectedJob ? '#c88c00' : '#8a8078', border: selectedJob ? '1px solid rgba(201,168,76,0.25)' : '1px solid rgba(200,140,0,0.12)', maxWidth: '160px' }}>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3" style={{ WebkitOverflowScrolling: 'touch' }}>
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12 px-4">
            <div className="w-14 h-14 rounded-full flex items-center justify-center mb-3" style={{ background: 'rgba(201,168,76,0.1)' }}>
              {agentMode === 'know-it-all' ? <Brain size={24} style={{ color: '#c88c00' }} /> : <FileSearch size={24} style={{ color: '#c88c00' }} />}
            </div>
            <h2 className="text-base font-bold mb-1" style={{ color: '#c88c00' }}>{agentMode === 'know-it-all' ? 'BKB Assistant' : 'Project Specs'}</h2>
            <p className="text-xs text-center mb-5 max-w-xs" style={{ color: '#8a8078' }}>
              ...
              {selectedJob && <span style={{ color: '#c88c00' }}>{' '}Focused on #{selectedJob.number}.</span>}
            </p>
            <div className="grid grid-cols-1 gap-2 w-full max-w-sm">
              {suggestions.map(s => (
                <button key={s} onClick={() => setQuery(s)} className="text-left px-3 py-2.5 rounded-lg text-xs active:brightness-110" style={{ background: '#ffffff', color: '#8a8078', border: '1px solid rgba(200,140,0,0.08)' }}>{s}</button>
              ))}
            </div>
          </div>
        )}
            ...
            <div className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} gap-2`}>
              {msg.role === 'assistant' && (
                <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-1" style={{ background: 'rgba(201,168,76,0.15)' }}>
                  <Bot size={12} style={{ color: '#c88c00' }} />
                </div>
              )}
        ...
        {loading && (
          <div className="flex justify-start gap-2">
            ...
            <div className="px-3 py-2.5 rounded-2xl flex items-center gap-2" style={{ background: '#f8f6f3', border: '1px solid rgba(200,140,0,0.08)', borderBottomLeftRadius: '4px' }}>
      ...
      <div className="flex-shrink-0 px-3 pb-[env(safe-area-inset-bottom,8px)] pt-2" style={{ background: '#111', borderTop: '1px solid rgba(200,140,0,0.1)' }}>
        {uploadedFiles.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-2">
            {uploadedFiles.map((file, idx) => (
              <div key={`${file.name}-${idx}`} className="flex items-center gap-1 px-2 py-1 rounded text-[10px]" style={{ background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.15)', color: '#c88c00' }}>
        ...
        <form onSubmit={handleSubmit} className="flex items-end gap-2">
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} className="p-2.5 rounded-xl flex-shrink-0 active:bg-white/5" style={{ color: uploadedFiles.length > 0 ? '#c88c00' : '#8a8078' }}>
            <Paperclip size={20} />
          </button>
          <div className="flex-1">
            <textarea
              ...
              style={{ background: '#f8f6f3', color: '#1a1a1a', border: selectedJob ? '1px solid rgba(201,168,76,0.25)' : '1px solid rgba(200,140,0,0.12)', minHeight: '42px', maxHeight: '120px', overflowY: 'auto' }}
              disabled={loading}
            />
          </div>
          <button type="submit" disabled={!canSend} className="p-2.5 rounded-xl flex-shrink-0 active:brightness-110"
            style={{ color: canSend ? '#111' : '#8a8078', background: canSend ? '#c88c00' : 'transparent' }}>
            <Send size={20} />
          </button>
        </form>
      ...
      {showDrawer && (
        <>
          ...
          <div className="fixed inset-y-0 left-0 z-50 w-[280px] flex flex-col" style={{ background: '#ffffff', boxShadow: '4px 0 24px rgba(0,0,0,0.5)' }}>
            <div className="flex items-center justify-between px-3 py-3" style={{ borderBottom: '1px solid rgba(200,140,0,0.1)' }}>
              <span className="text-sm font-bold" style={{ color: '#c88c00' }}>Conversations</span>
            ...
            <button onClick={handleNewChat} className="flex items-center gap-2 mx-3 mt-3 px-3 py-2.5 rounded-lg text-sm font-medium active:brightness-110"
              style={{ background: 'linear-gradient(135deg, rgba(201,168,76,0.2), rgba(201,168,76,0.1))', color: '#c88c00', border: '1px solid rgba(201,168,76,0.3)' }}>
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 space-y-1" style={{ WebkitOverflowScrolling: 'touch' }}>
              {convsLoading ? (
                ...
                <button key={conv.id} onClick={() => handleLoadConv(conv.id)} className="group w-full text-left px-3 py-2.5 rounded-lg transition-colors active:bg-white/5 flex flex-col relative"
                  style={{ background: conversationId === conv.id ? 'rgba(201,168,76,0.08)' : 'transparent', borderLeft: conversationId === conv.id ? '2px solid #c88c00' : '2px solid transparent' }}>
                  <span className="text-xs font-medium truncate w-full pr-8" style={{ color: conversationId === conv.id ? '#c88c00' : '#1a1a1a' }}>{conv.title}</span>
      ...
      {showJobPicker && (
        <>
          ...
          <div className="fixed inset-x-4 top-[15%] z-50 rounded-xl overflow-hidden max-h-[70vh] flex flex-col" style={{ background: '#ffffff', border: '1px solid rgba(200,140,0,0.15)', boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
            <div className="px-4 py-3" style={{ borderBottom: '1px solid rgba(200,140,0,0.1)' }}>
              <div className="text-sm font-bold mb-2" style={{ color: '#c88c00' }}>Select Project</div>
              <input type="text" placeholder="Search projects..." value={jobSearch} onChange={(e) => setJobSearch(e.target.value)} autoFocus className="w-full px-3 py-2 rounded-lg text-sm outline-none" style={{ background: '#f8f6f3', color: '#1a1a1a', border: '1px solid rgba(200,140,0,0.15)' }} />
            </div>
            <button onClick={() => { setSelectedJob(null); setShowJobPicker(false); setJobSearch(''); }} className="w-full text-left px-4 py-3 text-sm active:bg-[#f8f6f3]" style={{ color: '#8a8078', borderBottom: '1px solid rgba(200,140,0,0.08)' }}>No project (search all)</button>
            <div className="overflow-y-auto flex-1" style={{ WebkitOverflowScrolling: 'touch' }}>
              {jobsLoading ? (
                ...
                return filtered.map(job => (
                  <button key={job.id} onClick={() => { setSelectedJob(job); setShowJobPicker(false); setJobSearch(''); }} className="w-full text-left px-4 py-3 text-sm active:bg-[#f8f6f3] flex flex-col"
                    style={{ color: selectedJob?.id === job.id ? '#c88c00' : '#1a1a1a', background: selectedJob?.id === job.id ? 'rgba(201,168,76,0.06)' : 'transparent' }}>
