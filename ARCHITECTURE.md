...
ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ lib/
    ...
    ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ co-tracking.ts                # Shared CO tracking utility (document→budget linkage, used by all dashboards)
    ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ invoicing-health.ts           # Invoicing health analysis (contract + cost-plus, CC23 billable, released invoices, CO awareness)
...
- **Change Order awareness**: Calls shared `getCOTrackingForJob()` to detect approved COs, calculates `approvedCOValue` (all approved order value minus original contract), provides `totalContractAndCOValue` denominator for accurate billing progress, flags `unbilledCOAmount` for approved-but-uninvoiced COs
- **Invoicing UI CO indicators**: Contract job cards show "Contract: $X + Approved COs: $Y (N COs)" breakdown when COs exist, orange alert for uninvoiced CO amounts, progress bar uses CO-inclusive total as denominator
...
- **New Supabase migration `006_marketing_tables.sql`** adds 10 tables and 2 views:
  - `client_review_history` — single-source-of-truth for dedup ("never re-ask a client who's already reviewed")
  - `review_requests` — every ask, trigger, survey response, outcome
  - `review_responses` — drafted replies to incoming reviews, with approval workflow
  - `reputation_mentions` — web-sweep results
  - `newsletter_issues` / `newsletter_sections` / `newsletter_sends` — Phase 2 scaffold
  - `fb_posts` / `fb_drafts` — Phase 3 scaffold
  - `marketing_events` — unified agent activity log
  - Views: `marketing_approval_queue`, `review_funnel_90d`
- **New API routes:**
  - `GET /api/marketing/review-eligibility?contactId=X` — GHL calls this before sending; hard dedup check
  - `GET|POST /api/marketing/review-requests` — list/manually-queue
  - `GET|POST /api/marketing/review-responses` — approval queue for drafted replies
  - `GET /api/marketing/stats` — dashboard stats
  - `POST /api/webhook/ghl-survey-response` — GHL fires on survey response; branches 5★ vs 1-4★
  - `POST /api/webhook/new-review` — detected Google/Houzz/FB review recorded here, dedupes future asks
  - `GET /api/cron/review-concierge` — nightly scan for JobTread status changes (completion + post-design triggers)
- **New library modules** (`app/api/lib/marketing/`):
  - `review-dedup.ts` — `checkReviewEligibility`, `recordReviewLeft`
  - `ghl-workflows.ts` — fires GHL workflow for a contact by trigger type
  - `review-concierge.ts` — the agent brain: dedup → queue → GHL fire → log
- **Vercel cron** added: `/api/cron/review-concierge` runs daily at 07:00 UTC.
- **Env vars required** for Phase 1 to operate:
  - `GHL_WEBHOOK_SECRET`, `REVIEW_WEBHOOK_SECRET`
  - `GHL_REVIEW_WORKFLOW_COMPLETION_ID`, `GHL_REVIEW_WORKFLOW_NURTURE_ID`, `GHL_REVIEW_WORKFLOW_POST_DESIGN_ID`
  - `JT_COMPLETE_STATUS` (default: "Complete"), `JT_DESIGN_COMPLETE_STATUS` (default: "Design Phase Complete")
- **Files added**: 15 new files across `app/api/lib/marketing/`, `app/api/marketing/`, `app/api/webhook/`, `app/api/cron/review-concierge/`, `app/dashboard/marketing/`, `supabase/migrations/006_marketing_tables.sql`
- **Files modified**: `app/dashboard/layout.tsx` (Marketing nav), `vercel.json` (cron + function config)
- **Planning doc**: `/mnt/Claude/BKB App/BKB/BKB-Marketing-Agents-Plan.docx` (delivered separately to Nathan)
- **Phase 2 (Newsletter) and Phase 3 (Facebook)** have scaffolded tables and placeholder UI — not yet wired.
...
- **CO Tracking API** added to `GET /api/field-dashboard` response via shared `getCOTrackingForJob()` from `app/lib/co-tracking.ts`
...
- **CO Submission via Field-Staff Agent** — full change order workflow through inline Ask Agent:
  ...
  - Outputs `@@CO_PROPOSAL@@` structured JSON block parsed by chat route, displayed as approval table in UI
  - Pricing rules: $85/$125 BKB labor (32%), 30% margin on subs/materials
  - Group hierarchy: `🔁 Change Orders > CO: [Name]`
- **`create_change_order` tool** — REAL PAVE budget creation (not a stub):
  1. Fetches cost codes, cost types, units for ID resolution
  2. Finds or creates `🔁 Change Orders` parent budget group
  3. Creates CO subgroup with markdown description
  4. Creates cost items under the group via PAVE `createCostItem` mutations
  5. Uploads images to JobTread and attaches to CO cost group
  6. Optionally creates draft CO customerOrder document from the budget items
  7. Always creates follow-up task assigned to Nathan (3-day due date)
- **Image Upload Pipeline**:
  - New `POST /api/upload` endpoint — accepts multipart form data, stores in Supabase Storage (`co-photos` bucket)
  - Returns public URLs passed to JobTread's `createFile` PAVE mutation for attachment
  - UI: paperclip button in Ask Agent input, image preview strip with remove buttons, upload indicator
  - Images attached to CO cost group in JobTread → pulled into CO documents automatically
- **CO Proposal Approval UI**: parsed `@@CO_PROPOSAL@@` block shown as a table with item names, qty, cost, price, totals
  - Shows document creation flag, follow-up task details, photo count
  - Approve/Cancel buttons; on approval, sends structured JSON to agent for execution
- **Chat route**: now parses both `@@TASK_CONFIRM@@` and `@@CO_PROPOSAL@@` blocks from agent replies
- **Types**: Added `COProposalData`, `COLineItem` to `useAskAgent.ts`
- **Files changed**: `app/api/upload/route.ts` (new), `app/api/chat/route.ts`, `app/api/field-dashboard/route.ts`, `app/api/lib/agents/field-staff.ts`, `app/dashboard/field/page.tsx`, `app/hooks/useAskAgent.ts`, `app/lib/jobtread.ts`
...
### 2026-04-05 — Shared CO Tracking Utility + Invoicing CO Awareness

**What:** Extracted CO detection into shared utility, upgraded Terri's dashboard from name-matching to document→budget linkage, added CO awareness to invoicing dashboard.

**Changes:**
- `app/lib/co-tracking.ts` — **NEW** Shared CO tracking utility extracted from field dashboard. Exports `getCOTrackingForJob()` (4-phase document→budget linkage), `getCOTrackingForJobs()` (batch wrapper, parallel batches of 5), `normalizeCOName()`, and TypeScript interfaces (`BudgetCO`, `COTrackingResult`, `JobCOResult`)
- `app/lib/dashboard-data.ts` — Replaced inferior name-matching CO detection in `fetchARandCOData()` with shared `getCOTrackingForJobs()`. Removed ~60 lines of heuristic code (`approvedCOGroupNames` Set, `findCOGroups()` recursion, depth limits). Removed cost groups PAVE query from per-job batch loop (only document query needed for AR now).
- `app/lib/invoicing-health.ts` — Extended `ContractJobHealth` with 4 new fields: `approvedCOValue`, `totalContractAndCOValue`, `unbilledCOAmount`, `appliedCOsCount`. Updated `invoicedPercent` denominator to use CO-inclusive total. CO value calculation: all approved order value minus original contract value.
- `app/dashboard/invoicing/page.tsx` — Added CO breakdown row ("Contract: $X + Approved COs: $Y (N COs)"), orange alert for uninvoiced CO amounts, updated progress bar denominator to CO-inclusive total.
- `app/api/field-dashboard/route.ts` — Removed ~230 lines of local `getCOTrackingForJob()`, now imports from shared `app/lib/co-tracking.ts`.

**Key design decisions:**
- CO value heuristic: `approvedCOValue = allApprovedOrderValue - totalContractValue` (since document-level cost items don't carry cost group references in invoicing context)
- Regex fix: replaced Unicode property escapes (`\p{Emoji_Presentation}`) with broad non-ASCII strip (`/^[^\w\s]+\s*/g`) for TS target compatibility (shared utility doesn't use `@ts-nocheck`)
- CO fields optional (`?`) on page-level interface for backward compatibility
